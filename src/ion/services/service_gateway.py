...
class ServiceGateway(object):
    ...
    def process_gateway_request(self, service_name=None, operation=None):
        """
        Makes a secure call to a SciON service
        """
        ...
        try:
            ...
            # Apply service white list and black list for initial protection
            self.check_service_accessible(service_name)
            ...
            if request.method == "POST":
                ...
                # debug only
                #payload = '{"params": { "restype": "BankAccount", "lcstate": "", "name": "", "id_only": false } }'
    ...
    def create_attachment(self):
        try:
            ...
            ion_actor_id, expiry = self.get_governance_info_from_request('serviceRequest', json_params)
            ion_actor_id, expiry = self.validate_request(ion_actor_id, expiry)
            ...
            return self.gateway_error_response(e)
    ...
    def check_service_accessible(self, service_name):
    ...
    def build_message_headers(self, ion_actor_id, expiry):
        ...
        try:
            ...
            org_roles = self.org_client.find_all_roles_by_user(ion_actor_id, headers=self._get_gateway_headers())

            role_header = get_role_message_headers(org_roles)

            # Cache the roles by user id
            self.user_role_cache.put(ion_actor_id, role_header)
    ...
    def create_ion_object(self, object_params):
        """Creating and initializing an ION object from a dictionary of parameters."""
    ...
    def json_response(self, response_data):
        ...
        """
        resp_obj = json_dumps(response_data, default=self.ion_object_encoder, indent=None if request.is_xhr else 2)
        return self.response_class(resp_obj, mimetype='application/json')
    ...
    def ion_object_encoder(self, obj):
        return obj.__dict__
...
#   curl --data 'payload={"params": { "restype": "BankAccount", "lcstate": "", "name": "", "id_only": true } }' http://localhost:4000/service/request/resource_registry/find_resources
#
